...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    public void registerCommands() {
        ...
        // TODO: need /waradmin command for admin commands
        Objects.requireNonNull(plugin.getCommand("war")).setExecutor(this);
        Objects.requireNonNull(plugin.getCommand("war")).setTabCompleter(this);
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (cmd == null) {
            sender.sendMessage( "[SAW] " + ChatColor.RED + "Unknown command. Use /war for available commands.");
            showHelp(sender);
            return true;
        }
        ...
        if (!cmd.hasPermission(sender)) {
            sender.sendMessage("[SAW] " + ChatColor.RED + "You don't have permission to use this command.");
            return true;
        }

        // Check if player only command
        if (cmd.isPlayerOnly() && !cmd.isPlayer(sender)) {
            sender.sendMessage("[SAW] This command can only be used by players.");
            return true;
        }
    ...
    private boolean executeCommand(BaseCommand command, CommandSender sender, String[] args) {
        try {
            ...
            log(SEVERE, "[SAW] Error while executing command '" + command.getName() +
    ...
    private void showHelp(CommandSender sender) {
        ...
        for (BaseCommand cmd : commands.values()) {
            if (cmd.hasPermission(sender)) {
                sender.sendMessage(ChatColor.YELLOW + "/war " + cmd.getName() +
